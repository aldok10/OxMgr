...
fn serve_dashboard_config() -> HttpResponse {
    let interval_ms: u64 = std::env::var("OXMGR_DASHBOARD_INTERVAL_MS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(2000)
        .clamp(200, 10000);

    let label = std::env::var("OXMGR_DASHBOARD_LABEL").ok();
    let label_color = std::env::var("OXMGR_DASHBOARD_LABEL_COLOR").ok();
