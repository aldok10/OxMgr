...
use oxmgr_metrics::process::{ManagedProcess, StartProcessSpec};
...
pub enum IpcRequest {
    ...
    Logs { target: String },
    /// Return analysis findings and decisions, for every process or one named process.
    ///
    /// `target: None` means every process, matching how `Pull` already expresses "all or one" —
    /// rather than a separate variant, which would double the handling for one optional field.
    Findings { target: Option<String> },
}
...
pub struct IpcResponse {
    ...
    /// Analysis findings, for the `Findings` request.
    ///
    /// `#[serde(default)]` so an older client reading a newer daemon's response — or the reverse —
    /// still parses. The envelope is shared by every request, so a field that broke compatibility
    /// would break every command rather than only this one.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub findings: Option<FindingsReport>,
}

// Flattened findings wire types (`FindingsReport`, `FindingRow`,
// `DecisionRow`) moved to `oxmgr-manager` (workspace-crate-layout phase 6):
// the manager produces the report, so the types live where they are built.
// Re-exported here so IPC call sites keep their historical paths.

pub use oxmgr_manager::process_manager::FindingsReport;
...
mod tests {
    ...
    use super::{IpcRequest, IpcResponse, read_json_line, send_request, write_json_line};
