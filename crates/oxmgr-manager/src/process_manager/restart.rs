...
use oxmgr_metrics::process::{
    DesiredState, HealthStatus, ManagedProcess, ProcessExitEvent, ProcessStatus,
};
...
/// Read-only form of [`crash_loop_limit_reached`], for a caller that must not mutate.
///
/// The restart path prunes the history as it checks, because it is about to act and stale entries
/// would let a long-recovered process be refused. Analysis only *reports* the state, runs on every
/// tick, and iterates the process map immutably — pruning from there would mean taking a mutable
/// borrow of every process to answer a question, and would also silently change supervision state
/// from a read path.
///
/// So the window is applied as a filter instead of by mutation. Same predicate, same answer, no
/// side effect.
pub(super) fn crash_loop_limit_reached_at(process: &ManagedProcess, now: u64) -> bool {
    ...
    let recent = process
        .auto_restart_history
        .iter()
        .filter(|timestamp| now.saturating_sub(**timestamp) < CRASH_RESTART_WINDOW_SECS)
        .count();
    recent >= process.crash_restart_limit as usize
}
