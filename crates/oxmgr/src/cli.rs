...
pub enum Commands {
    ...
    Doctor,
    /// Show resource findings and the decisions they produced.
    ///
    /// Read-only. Nothing here acts on a process; protection mode is observe-only by default and
    /// is not reachable from this command at all.
    Findings {
        /// Only show findings for this process name or id. Defaults to every process.
        target: Option<String>,
        /// Emit JSON instead of a table.
        #[arg(long)]
        json: bool,
        /// Include findings whose condition has ended. Off by default, so the common question
        /// ("what is wrong now") is answered without the reader filtering the answer themselves.
        #[arg(long)]
        all: bool,
    },
...
mod tests {
    ...
    use super::{
        Cli, Commands, RestartArg, build_health_check, build_resource_limits, env_pairs_to_map,
        parse_env_var,
    };
    ...
    #[test]
    fn clap_parses_findings_with_defaults() {
        // No target means every process, matching how `pull` already expresses "all or one".
        let cli = Cli::parse_from(["oxmgr", "findings"]);
        match cli.command {
            Commands::Findings { target, json, all } => {
                assert_eq!(target, None);
                assert!(!json);
                // Cleared findings are off by default, so the common question — "what is wrong
                // now" — is answered without the reader filtering the answer themselves.
                assert!(!all);
            }
            other => panic!("expected findings subcommand, got {other:?}"),
        }
    }

    #[test]
    fn clap_parses_findings_with_target_and_flags() {
        let cli = Cli::parse_from(["oxmgr", "findings", "api", "--json", "--all"]);
        match cli.command {
            Commands::Findings { target, json, all } => {
                assert_eq!(target.as_deref(), Some("api"));
                assert!(json);
                assert!(all);
            }
            other => panic!("expected findings subcommand, got {other:?}"),
