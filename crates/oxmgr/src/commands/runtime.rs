...
use oxmgr_manager::signal::wait_for_shutdown_signal;
...
async fn terminate_child(child: &mut Child, signal_name: Option<&str>, timeout_window: Duration) {
    ...
    // The discards are deliberate: this is the last-resort force-kill path after the
    // grace window expired. ESRCH means the child already exited; any other failure
    // leaves nothing further this caller can do before the final reap below.
    #[expect(
        clippy::let_underscore_must_use,
        reason = "last-resort SIGKILL after grace window; ESRCH means already exited"
    )]
    ...
    #[expect(
        clippy::let_underscore_must_use,
        reason = "final reap after SIGKILL; a timeout here leaves the child to init's reaping"
    )]
...
async fn terminate_child(child: &mut Child, _signal_name: Option<&str>, timeout_window: Duration) {
    ...
    #[expect(
        clippy::let_underscore_must_use,
        reason = "last-resort kill after grace window; failure leaves the child to be reaped"
    )]
    let _ = child.start_kill();
    #[expect(
        clippy::let_underscore_must_use,
        reason = "final reap after kill; a timeout here leaves the child to init's reaping"
    )]
    let _ = timeout(Duration::from_secs(2), child.wait()).await;
}
...
async fn terminate_child(child: &mut Child, _signal_name: Option<&str>, _timeout_window: Duration) {
    #[expect(
        clippy::let_underscore_must_use,
        reason = "last-resort kill on an unsupported platform; nothing further can be done"
    )]
    let _ = child.start_kill();
    #[expect(
        clippy::let_underscore_must_use,
...
mod tests {
    ...
    use super::{RuntimeEntry, expand_specs_for_runtime, resolve_runtime_spawn, should_restart};
...
"#,
    ...
    fn test_config(prefix: &str) -> oxmgr_daemon::config::AppConfig {
        let base = temp_dir(prefix);
        oxmgr_daemon::config::AppConfig {
            base_dir: base.clone(),
