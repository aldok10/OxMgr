...
pub async fn run(command: Commands, config: &AppConfig) -> Result<()> {
    ...
    if needs_daemon {
        oxmgr_daemon::daemon::ensure_daemon_running(config).await?;
    }
    ...
    match command {
        ...
        Commands::Findings { target, json, all } => findings::run(config, target, json, all).await,
        Commands::Events {
            process,
            filter,
            json,
        ...
        } => anyhow::bail!("bug: daemon mode is handled before CLI dispatch"),
        Commands::Daemon {
            command: DaemonCommand::Stop,
