...
pub(crate) fn load_import_specs(
    ...
    match extension.as_deref() {
        ...
        _ => oxmgr_manager::ecosystem::load_with_profile(path, env),
    }
}
...
fn load_local_specs(source: &str, env: Option<&str>) -> Result<Vec<StartProcessSpec>> {
    ...
    if has_bundle_extension
        && usize::try_from(metadata.len()).unwrap_or(usize::MAX) > max_bundle_bytes() + 128
    {
        anyhow::bail!(
    ...
    if usize::try_from(metadata.len()).unwrap_or(usize::MAX) <= max_bundle_bytes() + 128 {
    ...
    for spec in specs {
        ...
        for idx in 0..instances {
            ...
            result.push(StartProcessSpec {
                ...
                // Carried through import, which is the point of 8.1: `depends_on` was already
                // parsed from ecosystem/Oxfile and consumed here only to compute start order, then
                // dropped. Correlation needs it on the runtime record, so it is preserved.
                //
                // NOT rewritten for instance expansion. When `instances > 1` this spec's own name
                // gains a `-{idx}` suffix, but a declared dependency names what the operator wrote,
                // and inventing `dep-0` for a dependency that may not be clustered would fabricate
                // an edge to a process that need not exist. An unresolvable name is reported as
...
mod tests {
    ...
    fn load_local_specs_reads_exported_bundle() {
        ...
        let spec = oxmgr_metrics::process::StartProcessSpec {
        ...
        let encoded = encode_bundle(&[oxmgr_metrics::process::ManagedProcess {
            id: 1,
            name: "api".to_string(),
            ...
            cgroup_path: None,
            pid: None,
            status: oxmgr_metrics::process::ProcessStatus::Stopped,
    ...
    #[test]
    fn ecosystem_import_carries_declared_dependencies_onto_the_spec() {
        // Driven through the real parse path rather than a hand-built spec, because the defect this
        // guards against was exactly that `depends_on` parsed correctly and then got dropped on the
        // way to `StartProcessSpec` — it was read only to compute start order. A fixture that
        // constructs the spec directly would not have caught that.
        let path = temp_file_path("import-depends-on", "json");
        fs::write(
            &path,
            r#"{"apps":[
                {"name":"db","script":"db.js"},
                {"name":"api","script":"server.js","depends_on":["db","not-managed"]},
                {"name":"solo","script":"solo.js"}
            ]}"#,
        )
        .expect("failed to write ecosystem fixture");

        let specs = load_local_specs(path.to_str().unwrap_or_default(), None)
            .expect("expected ecosystem import to parse");

        let api = specs
            .iter()
            .find(|spec| spec.name.as_deref() == Some("api"))
            .expect("api spec must be present");
        assert_eq!(
            api.depends_on,
            vec!["db".to_string(), "not-managed".to_string()],
            "declared dependencies must reach the start spec, in declaration order"
        );

        // An undeclared process reports none rather than inheriting a neighbour's edges.
        let solo = specs
            .iter()
            .find(|spec| spec.name.as_deref() == Some("solo"))
            .expect("solo spec must be present");
    ...
    #[test]
    fn instance_expansion_preserves_dependencies_as_declared() {
        // `instances: 2` suffixes this app's OWN name (`api-0`, `api-1`), but a declared dependency
        // names what the operator wrote. Rewriting "db" to "db-0" would fabricate an edge to a
        // process that need not exist, so the declaration is carried through untouched.
        let path = temp_file_path("import-depends-instances", "json");
        fs::write(
            &path,
            r#"{"apps":[
                {"name":"api","script":"server.js","instances":2,"depends_on":["db"]}
            ]}"#,
        )
        .expect("failed to write ecosystem fixture");
        ...
        assert_eq!(specs.len(), 2, "two instances expected");
        for spec in &specs {
            assert_eq!(
                spec.depends_on,
                vec!["db".to_string()],
                "each instance keeps the dependency as declared, unsuffixed"
            );
        }
