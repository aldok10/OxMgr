...
use oxmgr_manager::storage::PersistedState;
...
pub(crate) async fn run(config: &AppConfig) -> Result<()> {
    ...
    // Task 4.7: the protection actually in force, not the protection we asked for.
    //
    // The daemon sets 0o600 on the state file, and until now nothing could report what took effect —
    // so an operator had no way to distinguish a restricted file from one the platform silently left
    // open. On Windows the answer is "no restriction was applied at creation", which is a platform
    // difference rather than a fault, so it is reported as a warning naming the difference rather than
    // as a failure.
    if config.state_path.exists() {
        match oxmgr_manager::storage::inspect_protection(&config.state_path) {
            Ok(protection) => {
                let status = match protection.owner_only {
                    Some(true) => DoctorStatus::Ok,
                    // Owner-only is expressible here and is not in force: a state file readable
                    // beyond its owner exposes every managed process's environment.
                    Some(false) => DoctorStatus::Fail,
                    // The platform cannot answer. Not a fault, and not a clean bill of health either.
                    None => DoctorStatus::Warn,
                };
                record(status, "state_protection", protection.summary);
            }
            Err(err) => record(DoctorStatus::Warn, "state_protection", err.to_string()),
        }
    }
    ...
    let (platform_status, platform_detail) = inspect_platform_support();
    record(platform_status, "platform", platform_detail);

    let (advisory_status, advisory_detail) = inspect_config_advisories(&persisted_state.processes);
    record(advisory_status, "config_risk", advisory_detail);
...
/// Reports configuration risk advisories for every managed process.
///
/// The same `advisories::evaluate` the dashboard and the API will call, deliberately: task 3.5 asks
/// for one rule set so the CLI and the dashboard cannot drift into disagreeing about whether a
/// configuration is risky. Duplicating the rules here would guarantee that drift eventually.
///
/// Host memory is read for the capacity-relative class. Where it cannot be read the class is
/// withheld rather than answered — a limit that cannot be compared is not "fine".
///
/// A Critical advisory is a WARNING here, not a failure. `doctor` failing would make it exit
/// non-zero on a daemon that is running perfectly well with a configuration its operator chose;
/// advisories are informational, and acting on one is an explicit non-goal of the change that
/// defines them.
fn inspect_config_advisories(processes: &[ManagedProcess]) -> (DoctorStatus, String) {
    use oxmgr_manager::advisories::{AdvisorySeverity, HostCapacity, ProcessConfig, evaluate};

    if processes.is_empty() {
        return (DoctorStatus::Ok, "no managed processes".to_string());
    }

    // Read once for every process rather than per process: total memory does not change between
    // two lines of output, and refreshing per process would be work for an identical answer.
    let mut system = sysinfo::System::new();
    system.refresh_memory();
    let capacity = HostCapacity::from_total_memory_bytes(system.total_memory());
    ...
    for process in processes {
        let report = evaluate(&ProcessConfig::from(process), capacity);
        if !report.capacity.was_evaluated() {
            withheld = true;
        }
        for advisory in &report.advisories {
            match advisory.severity {
                AdvisorySeverity::Critical => critical += 1,
                AdvisorySeverity::Warning => warning += 1,
                AdvisorySeverity::Info => info += 1,
            }
            // Named with its consequence, not counted. A count tells an operator something is wrong
    ...
    if lines.is_empty() {
        let detail = if withheld {
            "no advisories; host capacity unavailable, so capacity-relative rules were withheld"
                .to_string()
        } else {
            format!("no advisories across {} process(es)", processes.len())
        };
        return (DoctorStatus::Ok, detail);
    }

    let mut counts = Vec::new();
    for (count, label) in [(critical, "critical"), (warning, "warning"), (info, "info")] {
        if count > 0 {
            counts.push(format!("{count} {label}"));
        }
    }
    let mut detail = counts.join(", ");
    if withheld {
        detail.push_str(" (capacity-relative rules withheld: host capacity unavailable)");
    }
    for line in lines {
...
/// Reports this platform's declared limitations from the support matrix.
///
/// `doctor` is where an operator already looks when the environment is the suspect, so a limitation
/// that is a queryable value in `src/platform.rs` belongs here rather than only in documentation.
/// The alternative — finding out that `oxmgr events` cannot work on Windows by running it — is what
/// the matrix exists to prevent.
///
/// A declared, understood limitation is a WARNING, not a failure: the daemon works, one capability
/// differs, and failing the check would make `doctor` exit non-zero on a perfectly healthy Windows
/// host. An `Unknown` is reported separately, because "nobody has established this" is a different
/// thing to tell an operator than "this does not work here".
fn inspect_platform_support() -> (DoctorStatus, String) {
    let Some(platform) = oxmgr_metrics::platform::current() else {
        // A target nothing is released for. Saying so is better than reporting a matrix that
        // describes three other platforms.
        return (
            DoctorStatus::Warn,
            "no support matrix for this target: platform-specific behaviour is undeclared"
                .to_string(),
        );
    };

    let limitations = oxmgr_metrics::platform::limitations(platform);
    if limitations.is_empty() {
        return (
            DoctorStatus::Ok,
            format!("{}: no declared limitations", platform.label()),
        );
    }

    let unknown = limitations
        .iter()
        .filter(|(_, support)| support.level == oxmgr_metrics::platform::SupportLevel::Unknown)
        .count();
    let known = limitations.len() - unknown;

    let mut detail = format!("{}: ", platform.label());
    let mut parts = Vec::new();
    if known > 0 {
        parts.push(format!("{known} declared limitation(s)"));
    }
    if unknown > 0 {
        parts.push(format!("{unknown} unverified"));
    }
    detail.push_str(&parts.join(", "));

    // The capabilities themselves, so the operator does not have to go and read the matrix. Named
    // rather than counted: a count tells them something is wrong without telling them what.
    for (declaration, support) in &limitations {
        detail.push_str(&format!(
            "\n                     - {} [{}]: {}",
            declaration.summary,
            support.level.label(),
            support.reason.unwrap_or("no reason recorded"),
        ));
    }
...
fn inspect_service_definition(system: InitSystem) -> (DoctorStatus, String) {
    match system {
        ...
        InitSystem::Auto => (
            DoctorStatus::Fail,
            "bug: Auto init system should have been resolved before diagnosis".to_string(),
        ),
    }
}
...
mod tests {
    ...
    use crate::cli::InitSystem;
    use oxmgr_metrics::process::{ManagedProcess, ProcessStatus, RestartPolicy};

    use super::{
        BindStatus, DoctorStatus, inspect_config_advisories, inspect_git_pull_setup,
        inspect_platform_support, parse_http_response, probe_bind_status, service_definition_path,
    };

    #[test]
    fn config_risk_check_names_advisories_and_never_fails() {
        // A configuration with crash-loop protection off and no restart delay: two Critical
        // advisories, and the pair the daemon can least afford together.
        ...
        let (status, detail) = inspect_config_advisories(&[process]);

        // Never a failure. `doctor` exiting non-zero here would fail on a daemon running perfectly
        // well with a configuration its operator chose, and acting on an advisory is an explicit
        // non-goal of the change that defines them.
        assert_eq!(
            status,
            DoctorStatus::Warn,
            "an advisory must warn, never fail the check"
        );
        assert!(
            detail.contains("critical"),
            "severity not summarised: {detail}"
        );
        // Named with its consequence rather than counted: a count says something is wrong without
        // saying what to do.
        assert!(
            detail.contains("api"),
            "the affected process must be named: {detail}"
        );
        assert!(
            detail.contains("circuit breaker can never trip"),
            "the consequence must be stated, not just the rule name: {detail}"
        );
    }

    #[test]
    fn config_risk_check_is_quiet_for_a_sound_configuration() {
        // Silence is the default. An operator who learns to skim this line will miss the run where
        // it matters.
        let mut process = sample_process();
        process.restart_policy = RestartPolicy::OnFailure;
        process.crash_restart_limit = 3;
        process.restart_delay_secs = 1;
        process.resource_limits = None;
        process.watch = false;

        let (status, detail) = inspect_config_advisories(&[process]);
        assert_eq!(status, DoctorStatus::Ok, "unexpected advisories: {detail}");
    }

    #[test]
    fn config_risk_check_handles_no_processes() {
        let (status, detail) = inspect_config_advisories(&[]);
        assert_eq!(status, DoctorStatus::Ok);
        assert!(detail.contains("no managed processes"));
    }

    #[test]
    fn platform_check_reports_declared_limitations_with_a_reason() {
        let (status, detail) = inspect_platform_support();

        // A declared limitation is a WARNING, never a failure: the daemon works, one capability
        // differs, and failing here would make `doctor` exit non-zero on a healthy host.
        assert!(
            matches!(status, DoctorStatus::Ok | DoctorStatus::Warn),
            "a platform limitation must never be reported as a failure, got {status:?}"
        );

        let Some(platform) = oxmgr_metrics::platform::current() else {
            // A target nothing is released for. The check says so rather than reporting a matrix
            // describing three other platforms.
            assert!(detail.contains("no support matrix"));
            return;
        };
        assert!(
            detail.contains(platform.label()),
            "the operator must be told which platform this is: {detail}"
        );

        let limitations = oxmgr_metrics::platform::limitations(platform);
        if limitations.is_empty() {
            assert_eq!(status, DoctorStatus::Ok);
            assert!(detail.contains("no declared limitations"));
            return;
        }
        ...
        assert_eq!(
            status,
            DoctorStatus::Warn,
            "a host with limitations must warn rather than pass silently"
        );
        // Named, not counted. A count tells an operator something is wrong without telling them
        // what, and every entry must carry a reason they can act on.
        for (declaration, support) in &limitations {
            assert!(
                detail.contains(declaration.summary),
                "limitation not named in the output: {}",
                declaration.summary
            );
            let reason = support.reason.expect("the matrix guarantees a reason here");
            assert!(
                detail.contains(reason),
                "reason omitted for {}: {detail}",
                declaration.summary
            );
        }
    }
    ...
    fn sample_process() -> ManagedProcess {
        ManagedProcess {
            ...
            desired_state: oxmgr_metrics::process::DesiredState::Stopped,
            last_exit_code: None,
            stdout_log: PathBuf::from("/tmp/stdout.log"),
            stderr_log: PathBuf::from("/tmp/stderr.log"),
            health_check: None,
            health_status: oxmgr_metrics::process::HealthStatus::Unknown,
            health_failures: 0,
