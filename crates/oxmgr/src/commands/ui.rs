...
use oxmgr_manager::logging::read_last_lines;
use oxmgr_metrics::process::ManagedProcess;

use self::input::{LoopControl, handle_key, handle_mouse};
#[cfg(test)]
use self::layout::{
...
impl Drop for TerminalGuard {
    fn drop(&mut self) {
        // Drop cannot return an error, and a failing terminal restore during teardown
        // leaves nothing for the caller to recover — the process is exiting anyway.
        #[expect(
            clippy::let_underscore_must_use,
            reason = "Drop cannot report; terminal restore is best-effort teardown"
        )]
...
impl ProcessFilter {
    ...
    fn matches(self, process: &ManagedProcess) -> bool {
        match self {
            Self::All => true,
            Self::Running => process.status == oxmgr_metrics::process::ProcessStatus::Running,
            Self::Stopped => process.status == oxmgr_metrics::process::ProcessStatus::Stopped,
            Self::Unhealthy => {
                process.health_status == oxmgr_metrics::process::HealthStatus::Unhealthy
            }
        }
    }
}
...
async fn submit_create_form(config: &AppConfig, state: &mut DashboardState) {
    ...
    let spec = oxmgr_metrics::process::StartProcessSpec {
        ...
        pre_reload_cmd: None,
        restart_policy: oxmgr_metrics::process::RestartPolicy::OnFailure,
        max_restarts: 10,
        ...
        // The TUI create form takes a command and a name; dependencies are declared in a config
        // file, so a process created here declares none.
        depends_on: Vec::new(),
    };
