...
fn expand_specs_for_apply(
    specs: Vec<oxmgr_manager::ecosystem::EcosystemProcessSpec>,
) -> Result<Vec<StartProcessSpec>> {
    let desired = expand_specs_with_deterministic_names(specs, "apply")?;
    let mut seen_names = HashSet::new();

    for spec in &desired {
        // SAFETY: expand_specs_with_deterministic_names guarantees name via line 71.
        #[expect(
            clippy::expect_used,
            reason = "expand_specs_with_deterministic_names guarantees name via line 71"
        )]
        let name = spec
            .name
            .as_ref()
            .expect("expand_specs_with_deterministic_names guarantees name");
    ...
    for spec in desired {
        ...
        if let Some(existing) = current_map.get(name) {
            if process_matches_spec(existing, spec) {
                if existing.status == oxmgr_metrics::process::ProcessStatus::Running
...
fn spec_name(spec: &StartProcessSpec) -> &str {
    // Every StartProcessSpec reaching this function was produced by
    // expand_specs_with_deterministic_names, which sets `name: Some(..)` at
    // common.rs:93. The Option exists only to carry the "unknown until expanded"
    // state at the ecosystem-spec boundary.
    #[expect(
        clippy::expect_used,
        reason = "expand_specs_with_deterministic_names sets name: Some(..) at common.rs:93"
    )]
    {
        spec.name
            .as_deref()
            .expect("expand_specs_with_deterministic_names sets name: Some(..) at common.rs:93")
    }
}
...
#[cfg(test)]
mod tests {
    use super::{ApplyAction, plan_apply_actions};
