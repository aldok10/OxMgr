...
pub fn load_full(path: &Path, profile: Option<&str>) -> Result<OxfileResult> {
    ...
    // Keep spec parsing: it validates the `[[apps]]` section even though
    // `load_full` only exposes the HTTP server section today.
    load_with_profile(path, profile)?;

    Ok(OxfileResult { http_server })
}
...
fn resolve_app(
    ...
    if let Some(profile_name) = profile
        && let Some(profile_map) = &app.profiles
        && let Some(profile_settings) = profile_map.get(profile_name)
    {
...
"#;
    ...
    fn write_from_specs_renders_toml() {
        ...
        assert!(rendered.contains("max_cpu_percent = 60"));
...
"#;
    ...
    fn load_with_profile_expands_env_and_cwd_tilde_and_vars() {
        let mut guard = crate::test_utils::EnvGuard::set("HOME", "/home/tester");
        guard.also_set("OXMGR_TEST_DATA", "shared");
        let _guard = guard;
...
"#;
    ...
    fn load_with_profile_errors_on_missing_env_variable() {
        let _guard = crate::test_utils::EnvGuard::remove("OXMGR_TEST_MISSING_VAR");
