...
struct BundleService {
    ...
    #[serde(default)]
    wait_ready: bool,
    #[serde(default = "oxmgr_metrics::process::default_ready_timeout_secs")]
    ...
    /// Declared dependencies, carried so an export/import round-trip preserves them.
    ///
    /// `#[serde(default)]` so a bundle written before this field existed still imports.
    #[serde(default)]
...
fn validate_service(service: &BundleService) -> Result<()> {
    ...
    if let Some(pre_reload_cmd) = service.pre_reload_cmd.as_ref()
        && (pre_reload_cmd.trim().is_empty() || pre_reload_cmd.len() > MAX_PROGRAM_LEN)
    {
    ...
    if let Some(secret_hash) = service.pull_secret_hash.as_deref()
        && (secret_hash.len() != 64 || !secret_hash.bytes().all(|ch| ch.is_ascii_hexdigit()))
    {
...
mod tests {
    ...
    use super::{BundleService, decode_bundle, encode_bundle, looks_like_bundle};
    use oxmgr_metrics::process::{
